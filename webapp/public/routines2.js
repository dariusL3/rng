...
// var scr = scrapeContract.at("0xad3df2e2587ebae6e3eb4a5536faa12010991f13")
...
function reg(){
  resetLocalStorage()
  unlock(myAddr)
  var newN = scr.getN.call();
  n = newN;
  for (j=0;j<n;j++){
    addressMap[scr.getParty.call(j)] = j;
  }
...
function listenAll(){
      ...
      if (Object.keys(temp).length==t){
        ...
        if (!opens[sharerInd]){
          ...
          if (openCount==commitCount){
            ...
            // var num = new web3.BigNumber(beacon)
            // document.getElementById("result").innerHTML = num.toFixed(0)
            //resetLocalStorage()
            return
          }
        }
  ...
  listeners[4].watch(function(err, result) {
    ...
    var ind = addressMap[result.args.partyAddress] ;
    console.log('Event : Public Key - '+ind+','+result.args.partyAddress)//+','+result.args.x+','+result.args.y);
...
function connect(){
  ...
  blockWatcher.watch(function(error,result){
    ...
    if (!error){
      watcherCounter += 1;
      if (watcherCounter >= sum) watcherCounter = 0;
      //web3.eth.getBlockNumber(function(e,blNum){
