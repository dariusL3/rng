...
contract scrape{
    ...
    function scrape(address addr) public{
        bn = bn128(addr);
        n = 4;
        ...
        parties = [540777616953818667695459266017912284570828942522, 219636331267287393984192082761220007907993627418, 989690846802841245981287344499810599510602531292, 47735750471354731554759357944171212818368730931];
        stage = 2;
    }
...
contract bn128{
    ...
    function g1mul(uint256 scalar, uint256 x, uint256 y) public view returns(uint256[2] p) {
      ...
      assembly {
        ...
        if iszero(staticcall(40001, 0x07, input, 0x60, p, 0x40)) {
          revert(12,12)
        }
      }
    }
    
    function g1add(uint256 x1, uint256 y1, uint256 x2, uint256 y2) public view returns(uint256[2] p) {
      ...
      assembly {
        // call ecmul precompile
        if iszero(staticcall(501, 0x06, input, 0x80, p, 0x40)) {
          revert(14,14)
        }
      }
    }
