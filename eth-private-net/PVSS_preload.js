...
function nextStage(){
  unlock()
  scr.nextStage.sendTransaction({from:eth.coinbase,value:0})
}
...
//   for (i=0;i<n;i++){scr.register.sendTransaction(arrayOfStuff[2*i],arrayOfStuff[2*i+1],{from:eth.coinbase,value:0});}
// }
...
function reg(){
  unlock()
  scr.register.sendTransaction(pkx[myKeyIndex],pky[myKeyIndex],{from:eth.coinbase,value:0})
}

function makeCommit(){
  n = scr.getN.call()
  t = scr.getT.call()
  if (earlyQuitters.indexOf(eth.coinbase)>=0) return;
  var i = addressMap[eth.coinbase]
  unlock()
  ...
  for (j=1;j<=n;j++){
    ...
    scr.share.sendTransaction(j,bundle[0].toString(10),bundle[1].toString(10),[bundle[2].toString(10),bundle[3].toString(10),bundle[4].toString(10),bundle[5].toString(10)],{from:eth.coinbase,value:0})
  ...
  scr.commit.sendTransaction(bundle[0],{from:eth.coinbase,value:0})
  //console.log(comm)
}
...
function listenAll(){
  ...
  listeners[3].watch(function(err, result) {
    ...
    for (i=1;i<=n;i++){
      ...
      console.log('Party '+i+" 's reveal missing")
    	var myInd = addressMap[eth.coinbase]
    	var w = web3.toBigNumber(web3.sha3(sks[i],2))%12345
    	// e, z, decx, decy
    	console.log("Producing decryption ")//+[sks[myKeyIndex],pks[myInd].x,pks[myInd].y,shareSet[myInd].x,shareSet[myInd].y,w])
    	var bundle = scr.computeDecryptionAndProof.call(sks[myKeyIndex],pkx[myKeyIndex],pky[myKeyIndex],shareSet[myInd].x,shareSet[myInd].y,w)
      scr.submitDecryption.sendTransaction(i, bundle[0], bundle[1], bundle[2], bundle[3],{from:eth.coinbase,value:0})
    	//console.log(bundle)
    }
  })
  ...
  listeners[5].watch(function(err, result) {
    ...
    console.log('Event : Share - '+result.args.shareIndex+','+partyInd+',')//+result.args.e+','+result.args.z+','+result.args.shx+','+result.args.shy+','+result.args.vx+','+result.args.vy)
    //console.log("Verifying with : "+[e,z,mypk.x,mypk.y,g[0],g[1],shx,shy,vx,vy])
    verifyResult = scr.verifyZKP.call(e,z,[mypk.x,mypk.y,g.x,g.y,shx,shy,vx,vy])
    //console.log([mypk.x,mypk.y,g.x,g.y,shx,shy,vx,vy])
    //console.log('Verify : '+verifyResult)
    ...
    if (verifyResult){
     ...
     if (Object.keys(temp).length==n){
     	...
     	if (commitCount==t){
     		if (lateQuitters.indexOf(eth.coinbase)>=0) return;
       		scr.open.sendTransaction(myOpen.x,myOpen.y,{from:eth.coinbase,value:0})
